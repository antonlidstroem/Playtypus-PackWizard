...
export default function StepExport({ ctx }) {
  ...
  const cats = (packMeta.categories || []).map(c =>
    typeof c === 'string' ? { id: c, label: c, emoji: '' } : c
  )
  ...
  const checks = [
    {
      ok: !!packMeta.packId,
      label: 'Pack-ID är satt',
      detail: packMeta.packId ? `packId: "${packMeta.packId}"` : 'Genereras automatiskt från appnamnet — gå till Grundinfo.',
    },
    {
      ok: !!packMeta.appName,
      label: 'Appnamn är satt',
      detail: packMeta.appName || 'Obligatoriskt — gå till Grundinfo.',
    },
    {
      ok: (languages || []).length > 0,
      label: 'Minst ett språk valt',
      detail: (languages || []).join(', ') || 'Gå till Grundinfo och välj språk.',
    },
    {
      ok: acts.length > 0 || (aiFields || []).includes('activities'),
      label: `Aktiviteter (${acts.length} manuella${(aiFields||[]).includes('activities') ? ' + AI' : ''})`,
      detail: acts.length === 0 && !(aiFields||[]).includes('activities')
        ? 'Lägg till aktiviteter eller markera "Låt AI generera" i Aktiviteter-steget.'
        : null,
    },
  ]

  // ── Varningar (inte blockerande) ──────────────────────────────────
  const warnings = [
    cats.length === 0
      && { label: 'Inga kategorier', detail: 'Aktiviteter saknar filtertabbar i appen.' },
    cats.some(c => !c.emoji)
      && { label: 'Kategorier saknar emoji', detail: cats.filter(c=>!c.emoji).map(c=>c.label).join(', ') },
    !packMeta.tagline
      && { label: 'Ingen tagline', detail: 'Headern blir tom under appnamnet.' },
    !packMeta.emoji
      && { label: 'Ingen app-emoji', detail: 'Standardemoji ✨ används.' },
    acts.filter(a => !a.steps?.length).length > 0
      && { label: `${acts.filter(a=>!a.steps?.length).length} aktiviteter saknar steg`, detail: 'Krävs för guidad visning och TTS.' },
    (aiFields || []).length > 0
      && { label: `${aiFields.length} olösta AI-fält`, detail: `Kör AI-prompten för att fylla i: ${aiFields.join(', ')}` },
  ].filter(Boolean)
    ...
    <div className="animate-fade">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', marginBottom: 14,
            textTransform: 'uppercase', letterSpacing: '0.05em' }}>Sammanfattning</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              ['Panikknapp',   packMeta.panicButton?.enabled
                ? `På — krav: ${[
                    packMeta.panicButton.maxPrepMinutes != null && `prep ≤${packMeta.panicButton.maxPrepMinutes}min`,
                    packMeta.panicButton.requireNoProps && 'ingen rekvisita',
                    packMeta.panicButton.maxDurationSeconds && `max ${Math.round(packMeta.panicButton.maxDurationSeconds/60)}min`,
                  ].filter(Boolean).join(', ') || 'slumpmässig'}`
                : 'Av'],
            ].map(([k, v]) => (
              <div key={k} style={{ display: 'flex', gap: 8, fontSize: 13 }}>
                ...
                <span style={{
                  ...
                  fontFamily: k === 'Pack-ID' || k === 'Accentfärg' ? 'DM Mono, monospace' : 'inherit',
                  fontSize: k === 'Pack-ID' || k === 'Accentfärg' ? 12 : 13,
                }}>{v}</span>
              </div>
            ))}
          </div>
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', marginBottom: 14,
            textTransform: 'uppercase', letterSpacing: '0.05em' }}>Validering</h3>
          {checks.map((c, i) => <ValidationItem key={i} {...c} />)}
          {warnings.length > 0 && (
            <>
              <div style={{ marginTop: 12, marginBottom: 8, fontSize: 11, fontWeight: 600,
                color: '#F7B731', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                ⚠️ Varningar
              </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 10 }}>
          Välj var appen ska deployas — genererar rätt GitHub Actions YAML i ZIP:en.
        </p>
      ...
      <div style={{
        ...
        border: '1px solid var(--border)', borderRadius: 'var(--radius)', marginBottom: 24,
      }}>
        <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', marginBottom: 10,
          textTransform: 'uppercase', letterSpacing: '0.05em' }}>ZIP innehåller</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {[
            ...
            <div key={f} style={{
              fontSize: 12, color: 'var(--text-2)', fontFamily: 'DM Mono, monospace',
      ...
      <div style={{
        padding: '14px 18px', background: 'var(--bg-card)',
        border: '1px solid var(--border)', borderRadius: 'var(--radius)',
        marginBottom: 24, fontSize: 13, lineHeight: 1.7, color: 'var(--text-2)',
      }}>
        ...
        <ol style={{ paddingLeft: 18, marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <li>Kopiera <code style={{ fontFamily: 'DM Mono', fontSize: 11, color: 'var(--accent)' }}>deploy-{packId}.yml</code> till <code style={{ fontFamily: 'DM Mono', fontSize: 11, color: 'var(--accent)' }}>.github/workflows/</code> i ditt repo</li>
          <li>Lägg till secret <code style={{ fontFamily: 'DM Mono', fontSize: 11, color: 'var(--accent)' }}>CONTENT_REPO_TOKEN</code> i GitHub</li>
      ...
      {!canExport && (
        <p style={{ marginTop: 10, fontSize: 12, color: 'var(--red)' }}>
          Åtgärda de markerade valideringsfelen ovan innan export.
        </p>
      )}
    </div>
  )
